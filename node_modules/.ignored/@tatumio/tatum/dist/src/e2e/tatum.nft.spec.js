...
describe.skip('Tatum NFT - Test mint flow', () => {
    it('Test', async () => {
        ...
        const client = await service_1.TatumSDK.init({
            network: service_1.Network.POLYGON_AMOY,
        });
        const result = await client.nft.createNftCollection({
            name: 'Test contract',
            symbol: 'AMOY_TEST',
        ...
        const scAddress = await util_1.Utils.retryWithTimeout(() => {
        ...
        await util_1.Utils.retryWithTimeout(() => {
            return client.rpc.getTransactionByHash(nft1.data.txId);
        }, 20000);
        const nft2 = await util_1.Utils.retryWithTimeout(async () => {
        ...
        const nft3 = await util_1.Utils.retryWithTimeout(async () => {
            const res = await client.nft.mintNft({
                to: toAddress,
                url: 'ipfs://bafkreiallhajtd2j57lkuufjtabe6be2m3qh672nmnmkhvttv6d44cezaa',
                contractAddress: scAddress ?? '',
