(function (global, factory) {
  ...
  var ErrorCode;
  (function (ErrorCode) {
      ErrorCode[ErrorCode["ZERO_ERROR"] = 0] = "ZERO_ERROR";
      ErrorCode[ErrorCode["SUCCESS"] = 1] = "SUCCESS";
      ErrorCode[ErrorCode["FAILURE"] = 2] = "FAILURE";
      ErrorCode[ErrorCode["INVALID_FORMAT_ERROR"] = 3] = "INVALID_FORMAT_ERROR";
      ErrorCode[ErrorCode["ILLEGAL_ARGUMENT_ERROR"] = 4] = "ILLEGAL_ARGUMENT_ERROR";
      ErrorCode[ErrorCode["MEMORY_ALLOCATION_ERROR"] = 5] = "MEMORY_ALLOCATION_ERROR";
  })(ErrorCode || (ErrorCode = {}));
  ...
  (function (RestrictionLevel) {
       ...
       * @stable ICU 51
       */
      RestrictionLevel[RestrictionLevel["ASCII"] = 268435456] = "ASCII";
      /**
       ...
       * @stable ICU 53
       */
      RestrictionLevel[RestrictionLevel["SINGLE_SCRIPT_RESTRICTIVE"] = 536870912] = "SINGLE_SCRIPT_RESTRICTIVE";
      /**
       ...
       * @stable ICU 51
       */
      RestrictionLevel[RestrictionLevel["HIGHLY_RESTRICTIVE"] = 805306368] = "HIGHLY_RESTRICTIVE";
      /**
       ...
       * @stable ICU 51
       */
      RestrictionLevel[RestrictionLevel["MODERATELY_RESTRICTIVE"] = 1073741824] = "MODERATELY_RESTRICTIVE";
      /**
       * All characters in the string are in the identifier profile.  Allow arbitrary mixtures of scripts.
       *
       * @stable ICU 51
       */
      RestrictionLevel[RestrictionLevel["MINIMALLY_RESTRICTIVE"] = 1342177280] = "MINIMALLY_RESTRICTIVE";
      /**
       * Any valid identifiers, including characters outside of the Identifier Profile.
       *
       * @stable ICU 51
       */
      RestrictionLevel[RestrictionLevel["UNRESTRICTIVE"] = 1610612736] = "UNRESTRICTIVE";
      /**
       * Mask for selecting the Restriction Level bits from the return value of {@link uspoof_check}.
       *
       * @stable ICU 53
       */
      RestrictionLevel[RestrictionLevel["RESTRICTION_LEVEL_MASK"] = 2130706432] = "RESTRICTION_LEVEL_MASK";
      /**
       * An undefined restriction level.
       * @internal
       */
      RestrictionLevel[RestrictionLevel["UNDEFINED_RESTRICTIVE"] = -1] = "UNDEFINED_RESTRICTIVE";
  })(RestrictionLevel || (RestrictionLevel = {}));
  var SpoofChecks;
  (function (SpoofChecks) {
      SpoofChecks[SpoofChecks["SINGLE_SCRIPT_CONFUSABLE"] = 1] = "SINGLE_SCRIPT_CONFUSABLE";
      SpoofChecks[SpoofChecks["MIXED_SCRIPT_CONFUSABLE"] = 2] = "MIXED_SCRIPT_CONFUSABLE";
      SpoofChecks[SpoofChecks["WHOLE_SCRIPT_CONFUSABLE"] = 4] = "WHOLE_SCRIPT_CONFUSABLE";
      SpoofChecks[SpoofChecks["CONFUSABLE"] = 7] = "CONFUSABLE";
      SpoofChecks[SpoofChecks["RESTRICTION_LEVEL"] = 16] = "RESTRICTION_LEVEL";
      SpoofChecks[SpoofChecks["INVISIBLE"] = 32] = "INVISIBLE";
      SpoofChecks[SpoofChecks["CHAR_LIMIT"] = 64] = "CHAR_LIMIT";
      SpoofChecks[SpoofChecks["MIXED_NUMBERS"] = 128] = "MIXED_NUMBERS";
      SpoofChecks[SpoofChecks["ALL_CHECKS"] = 65535] = "ALL_CHECKS";
      // AUX_INFO = 0x40000000,
  ...
  */
  ...
  var concat$1 = function (it) {
    var own = it.concat;
    return it === ArrayPrototype$1 || (objectIsPrototypeOf(ArrayPrototype$1, it) && own === ArrayPrototype$1.concat) ? concat : own;
  };
  ...
  var includes$2 = function (it) {
    var own = it.includes;
    if (it === ArrayPrototype$5 || (objectIsPrototypeOf(ArrayPrototype$5, it) && own === ArrayPrototype$5.includes)) return includes;
  ...
  var sort$1 = function (it) {
    var own = it.sort;
    return it === ArrayPrototype$6 || (objectIsPrototypeOf(ArrayPrototype$6, it) && own === ArrayPrototype$6.sort) ? sort : own;
  };
  ...
  var reduce$1 = function (it) {
    var own = it.reduce;
    return it === ArrayPrototype$8 || (objectIsPrototypeOf(ArrayPrototype$8, it) && own === ArrayPrototype$8.reduce) ? reduce : own;
  };
  ...
  const deviation = /[\u00df\u03c2\u200c\u200d]/;
  const nonAsciiLatin = /[\u0080–\u024f]/;
  const kanaCharacterException = /[\u3078-\u307a\u30d8-\u30da\u30fb-\u30fe]/;
  const combiningDiacriticException = /[\u0300-\u0339]/;
  const cyrillicLikeLatin = /[асԁеһіјӏорԛѕԝхуъЬҽпгѵѡ]/;
  // const scriptExtensionsInherited = '[\u200c-\u200d]'
  const identifierStatusAllowed = '[\u0300-\u0339]';
  const latinGreekCyrillicAscii = X(`([\\p{Latin}][\\p{Greek}][\\p{Cyrillic}][0-9\u002e_\u002d]${identifierStatusAllowed})`);
  const recommended = X('[0-9A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u0131\\u0134-\\u013E\\u0141-\\u0148\\u014A-\\u017E\\u018F\\u01A0\\u01A1\\u01AF\\u01B0\\u01CD-\\u01DC\\u01DE-\\u01E3\\u01E6-\\u01F0\\u01F4\\u01F5\\u01F8-\\u021B\\u021E\\u021F\\u0226-\\u0233\\u0259\\u02BB\\u02BC\\u02EC\\u0300-\\u0304\\u0306-\\u030C\\u030F-\\u0311\\u0313\\u0314\\u031B\\u0323-\\u0328\\u032D\\u032E\\u0330\\u0331\\u0335\\u0338\\u0339\\u0342\\u0345\\u037B-\\u037D\\u0386\\u0388-\\u038A\\u038C\\u038E-\\u03A1\\u03A3-\\u03CE\\u03FC-\\u045F\\u048A-\\u0529\\u052E\\u052F\\u0531-\\u0556\\u0559\\u0561-\\u0586\\u05B4\\u05D0-\\u05EA\\u05F0-\\u05F2\\u0620-\\u063F\\u0641-\\u0655\\u0660-\\u0669\\u0670-\\u0672\\u0674\\u0679-\\u068D\\u068F-\\u06D3\\u06D5\\u06E5\\u06E6\\u06EE-\\u06FC\\u06FF\\u0750-\\u07B1\\u08A0-\\u08AC\\u08B2\\u08B6-\\u08BD\\u0901-\\u094D\\u094F\\u0950\\u0956\\u0957\\u0960-\\u0963\\u0966-\\u096F\\u0971-\\u0977\\u0979-\\u097F\\u0981-\\u0983\\u0985-\\u098C\\u098F\\u0990\\u0993-\\u09A8\\u09AA-\\u09B0\\u09B2
\\uA7AE\\uA7FA\\uA9E7-\\uA9FE\\uAA60-\\uAA76\\uAA7A-\\uAA7F\\uAB01-\\uAB06\\uAB09-\\uAB0E\\uAB11-\\uAB16\\uAB20-\\uAB26\\uAB28-\\uAB2E\\uAC00-\\uD7A3\\uFA0E\\uFA0F\\uFA11\\uFA13\\uFA14\\uFA1F\\uFA21\\uFA23\\uFA24\\uFA27-\\uFA29\\u20000-\\u2A6D6\\u2A700-\\u2B734\\u2B740-\\u2B81D\\u2B820-\\u2CEA1]');
  const inclusion = X("['\\-.\\:\\u00B7\\u0375\\u058A\\u05F3\\u05F4\\u06FD\\u06FE\\u0F0B\\u200C\\u200D\\u2010\\u2019\\u2027\\u30A0\\u30FB]");
  const allowed = X.union([inclusion, recommended], 'i');
  const removed = X('[\\u0338\\u058a\\u2010\\u2019\\u2027\\u30a0\\u02bb\\u02bc\\u0620\\u0F8C\\u0F8D\\u0F8E\\u0F8F\\u01CD-\\u01DC\\u1C80-\\u1C8F\\u1E00-\\u1E9B\\u1F00-\\u1FFF\\uA640-\\uA69F\\uA720-\\uA7FF]');

  // import * as X from 'xregexp';
  const inBasicLatin = X('\\p{InBasic_Latin}'); // U+0000–U+007F
  const inLatin1Supplement = X('\\p{InLatin-1_Supplement}'); // U+0080–U+00FF
  const inLatinExtendedA = X('\\p{InLatin_Extended-A}'); // U+0100–U+017F
  const inLatinExtendedB = X('\\p{InLatin_Extended-B}'); // U+0180–U+024F
  const inIpaExtensions = X('\\p{InIPA_Extensions}'); // U+0250–U+02AF
  const inSpacingModifierLetters = X('\\p{InSpacing_Modifier_Letters}'); // U+02B0–U+02FF
  const inCombiningDiacriticalMarks = X('\\p{InCombining_Diacritical_Marks}'); // U+0300–U+036F
  const inGreekAndCoptic = X('\\p{InGreek_and_Coptic}'); // U+0370–U+03FF
  const inCyrillic = X('\\p{InCyrillic}'); // U+0400–U+04FF
  // export const inCyrillicSupplementary = X('\\p{InCyrillic_Supplementary}'); // U+0500–U+052F
  const inArmenian = X('\\p{InArmenian}'); // U+0530–U+058F
  const inHebrew = X('\\p{InHebrew}'); // U+0590–U+05FF
  const inArabic = X('\\p{InArabic}'); // U+0600–U+06FF
  const inSyriac = X('\\p{InSyriac}'); // U+0700–U+074F
  const inThaana = X('\\p{InThaana}'); // U+0780–U+07BF
  const inDevanagari = X('\\p{InDevanagari}'); // U+0900–U+097F
  const inBengali = X('\\p{InBengali}'); // U+0980–U+09FF
  const inGurmukhi = X('\\p{InGurmukhi}'); // U+0A00–U+0A7F
  const inGujarati = X('\\p{InGujarati}'); // U+0A80–U+0AFF
  const inOriya = X('\\p{InOriya}'); // U+0B00–U+0B7F
  const inTamil = X('\\p{InTamil}'); // U+0B80–U+0BFF
  const inTelugu = X('\\p{InTelugu}'); // U+0C00–U+0C7F
  const inKannada = X('\\p{InKannada}'); // U+0C80–U+0CFF
  const inMalayalam = X('\\p{InMalayalam}'); // U+0D00–U+0D7F
  const inSinhala = X('\\p{InSinhala}'); // U+0D80–U+0DFF
  const inThai = X('\\p{InThai}'); // U+0E00–U+0E7F
  const inLao = X('\\p{InLao}'); // U+0E80–U+0EFF
  const inTibetan = X('\\p{InTibetan}'); // U+0F00–U+0FFF
  const inMyanmar = X('\\p{InMyanmar}'); // U+1000–U+109F
  const inGeorgian = X('\\p{InGeorgian}'); // U+10A0–U+10FF
  const inHangulJamo = X('\\p{InHangul_Jamo}'); // U+1100–U+11FF
  const inEthiopic = X('\\p{InEthiopic}'); // U+1200–U+137F
  const inCherokee = X('\\p{InCherokee}'); // U+13A0–U+13FF
  const inUnifiedCanadianAboriginalSyllabics = X('\\p{InUnified_Canadian_Aboriginal_Syllabics}'); // U+1400–U+167F
  const inOgham = X('\\p{InOgham}'); // U+1680–U+169F
  const inRunic = X('\\p{InRunic}'); // U+16A0–U+16FF
  const inTagalog = X('\\p{InTagalog}'); // U+1700–U+171F
  const inHanunoo = X('\\p{InHanunoo}'); // U+1720–U+173F
  const inBuhid = X('\\p{InBuhid}'); // U+1740–U+175F
  const inTagbanwa = X('\\p{InTagbanwa}'); // U+1760–U+177F
  const inKhmer = X('\\p{InKhmer}'); // U+1780–U+17FF
  const inMongolian = X('\\p{InMongolian}'); // U+1800–U+18AF
  const inLimbu = X('\\p{InLimbu}'); // U+1900–U+194F
  const inTaiLe = X('\\p{InTai_Le}'); // U+1950–U+197F
  const inKhmerSymbols = X('\\p{InKhmer_Symbols}'); // U+19E0–U+19FF
  const inPhoneticExtensions = X('\\p{InPhonetic_Extensions}'); // U+1D00–U+1D7F
  const inLatinExtendedAdditional = X('\\p{InLatin_Extended_Additional}'); // U+1E00–U+1EFF
  const inGreekExtended = X('\\p{InGreek_Extended}'); // U+1F00–U+1FFF
  const inGeneralPunctuation = X('\\p{InGeneral_Punctuation}'); // U+2000–U+206F
  const inSuperscriptsAndSubscripts = X('\\p{InSuperscripts_and_Subscripts}'); // U+2070–U+209F
  const inCurrencySymbols = X('\\p{InCurrency_Symbols}'); // U+20A0–U+20CF
  const inCombiningDiacriticalMarksForSymbols = X('\\p{InCombining_Diacritical_Marks_for_Symbols}'); // U+20D0–U+20FF
  const inLetterlikeSymbols = X('\\p{InLetterlike_Symbols}'); // U+2100–U+214F
  const inNumberForms = X('\\p{InNumber_Forms}'); // U+2150–U+218F
  const inArrows = X('\\p{InArrows}'); // U+2190–U+21FF
  const inMathematicalOperators = X('\\p{InMathematical_Operators}'); // U+2200–U+22FF
  const inMiscellaneousTechnical = X('\\p{InMiscellaneous_Technical}'); // U+2300–U+23FF
  const inControlPictures = X('\\p{InControl_Pictures}'); // U+2400–U+243F
  const inOpticalCharacterRecognition = X('\\p{InOptical_Character_Recognition}'); // U+2440–U+245F
  const inEnclosedAlphanumerics = X('\\p{InEnclosed_Alphanumerics}'); // U+2460–U+24FF
  const inBoxDrawing = X('\\p{InBox_Drawing}'); // U+2500–U+257F
  const inBlockElements = X('\\p{InBlock_Elements}'); // U+2580–U+259F
  const inGeometricShapes = X('\\p{InGeometric_Shapes}'); // U+25A0–U+25FF
  const inMiscellaneousSymbols = X('\\p{InMiscellaneous_Symbols}'); // U+2600–U+26FF
  const inDingbats = X('\\p{InDingbats}'); // U+2700–U+27BF
  const inMiscellaneousMathematicalSymbolsA = X('\\p{InMiscellaneous_Mathematical_Symbols-A}'); // U+27C0–U+27EF
  const inSupplementalArrowsA = X('\\p{InSupplemental_Arrows-A}'); // U+27F0–U+27FF
  const inBraillePatterns = X('\\p{InBraille_Patterns}'); // U+2800–U+28FF
  const inSupplementalArrowsB = X('\\p{InSupplemental_Arrows-B}'); // U+2900–U+297F
  const inMiscellaneousMathematicalSymbolsB = X('\\p{InMiscellaneous_Mathematical_Symbols-B}'); // U+2980–U+29FF
  const inSupplementalMathematicalOperators = X('\\p{InSupplemental_Mathematical_Operators}'); // U+2A00–U+2AFF
  const inMiscellaneousSymbolsAndArrows = X('\\p{InMiscellaneous_Symbols_and_Arrows}'); // U+2B00–U+2BFF
  const inCjkRadicalsSupplement = X('\\p{InCJK_Radicals_Supplement}'); // U+2E80–U+2EFF
  const inKangxiRadicals = X('\\p{InKangxi_Radicals}'); // U+2F00–U+2FDF
  const inIdeographicDescriptionCharacters = X('\\p{InIdeographic_Description_Characters}'); // U+2FF0–U+2FFF
  const inCjkSymbolsAndPunctuation = X('\\p{InCJK_Symbols_and_Punctuation}'); // U+3000–U+303F
  const inHiragana = X('\\p{InHiragana}'); // U+3040–U+309F
  const inKatakana = X('\\p{InKatakana}'); // U+30A0–U+30FF
  const inBopomofo = X('\\p{InBopomofo}'); // U+3100–U+312F
  const inHangulCompatibilityJamo = X('\\p{InHangul_Compatibility_Jamo}'); // U+3130–U+318F
  const inKanbun = X('\\p{InKanbun}'); // U+3190–U+319F
  const inBopomofoExtended = X('\\p{InBopomofo_Extended}'); // U+31A0–U+31BF
  const inKatakanaPhoneticExtensions = X('\\p{InKatakana_Phonetic_Extensions}'); // U+31F0–U+31FF
  const inEnclosedCjkLettersAndMonths = X('\\p{InEnclosed_CJK_Letters_and_Months}'); // U+3200–U+32FF
  const inCjkCompatibility = X('\\p{InCJK_Compatibility}'); // U+3300–U+33FF
  const inCjkUnifiedIdeographsExtensionA = X('\\p{InCJK_Unified_Ideographs_Extension_A}'); // U+3400–U+4DBF
  const inYijingHexagramSymbols = X('\\p{InYijing_Hexagram_Symbols}'); // U+4DC0–U+4DFF
  const inCJKUnifiedIdeographs = X('\\p{InCJK_Unified_Ideographs}'); // U+4E00–U+9FFF
  const inYiSyllables = X('\\p{InYi_Syllables}'); // U+A000–U+A48F
  const inYiRadicals = X('\\p{InYi_Radicals}'); // U+A490–U+A4CF
  const inHangulSyllables = X('\\p{InHangul_Syllables}'); // U+AC00–U+D7AF
  const inHighSurrogates = X('\\p{InHigh_Surrogates}'); // U+D800–U+DB7F
  const inHighPrivateUseSurrogates = X('\\p{InHigh_Private_Use_Surrogates}'); // U+DB80–U+DBFF
  const inLowSurrogates = X('\\p{InLow_Surrogates}'); // U+DC00–U+DFFF
  const inPrivateUseArea = X('\\p{InPrivate_Use_Area}'); // U+E000–U+F8FF
  const inCjkCompatibilityIdeographs = X('\\p{InCJK_Compatibility_Ideographs}'); // U+F900–U+FAFF
  const inAlphabeticPresentationForms = X('\\p{InAlphabetic_Presentation_Forms}'); // U+FB00–U+FB4F
  const inArabicPresentationFormsA = X('\\p{InArabic_Presentation_Forms-A}'); // U+FB50–U+FDFF
  const inVariationSelectors = X('\\p{InVariation_Selectors}'); // U+FE00–U+FE0F
  const inCombiningHalfMarks = X('\\p{InCombining_Half_Marks}'); // U+FE20–U+FE2F
  const inCjkCompatibilityForms = X('\\p{InCJK_Compatibility_Forms}'); // U+FE30–U+FE4F
  const inSmallFormVariants = X('\\p{InSmall_Form_Variants}'); // U+FE50–U+FE6F
  const inArabicPresentationFormsB = X('\\p{InArabic_Presentation_Forms-B}'); // U+FE70–U+FEFF
  const inHalfwidthAndFullwidthForms = X('\\p{InHalfwidth_and_Fullwidth_Forms}'); // U+FF00–U+FFEF
  const inSpecials = X('\\p{InSpecials}'); // U+FFF0–U+FFFF
  ...
  const letter = X('\\p{Letter}'); // any kind of letter from any language.
  const lowercaseLetter = X('\\p{Lowercase_Letter}'); // a lowercase letter that has an uppercase variant.
  const uppercaseLetter = X('\\p{Uppercase_Letter}'); // an uppercase letter that has a lowercase variant.
  const titlecaseLetter = X('\\p{Titlecase_Letter}'); // a letter that appears at the start of a word when only the first letter of the word is capitalized.
  const casedLetter = X('\\p{Cased_Letter}'); // a letter that exists in lowercase and uppercase variants (combination of Ll, Lu and Lt).
  const modifiedLetter = X('\\p{Modifier_Letter}'); // a special character that is used like a letter.
  const otherLetter = X('\\p{Other_Letter}'); // a letter or ideograph that does not have lowercase and uppercase variants.
  const mark = X('\\p{Mark}'); // a character intended to be combined with another character (e.g. accents, umlauts, enclosing boxes, etc.).
  const nonSpacingMark = X('\\p{Non_Spacing_Mark}'); // a character intended to be combined with another character without taking up extra space (e.g. accents, umlauts, etc.).
  ...
  const enclosingMark = X('\\p{Enclosing_Mark}'); // a character that encloses the character is is combined with (circle, square, keycap, etc.).
  const separator = X('\\p{Separator}'); // any kind of whitespace or invisible separator.
  const spaceSeparator = X('\\p{Space_Separator}'); // a whitespace character that is invisible, but does take up space.
  const lineSeparator = X('\\p{Line_Separator}'); // line separator character U+2028.
  const paragraphSeparator = X('\\p{Paragraph_Separator}'); // paragraph separator character U+2029.
  const symbol$5 = X('\\p{Symbol}'); // math symbols, currency signs, dingbats, box-drawing characters, etc.
  const mathSymbol = X('\\p{Math_Symbol}'); // any mathematical symbol.
  const currencySymbol = X('\\p{Currency_Symbol}'); // any currency sign.
  const modifierSymbol = X('\\p{Modifier_Symbol}'); // a combining character (mark) as a full character on its own.
  const otherSymbol = X('\\p{Other_Symbol}'); // various symbols that are not math symbols, currency signs, or combining characters.
  /* tslint:disable-next-line */
  const number = X('\\p{Number}'); // any kind of numeric character in any script.
  const decimalDigitNumber = X('\\p{Nd}'); // a digit zero through nine in any script except ideographic scripts.
  const letterNumber = X('\\p{Letter_Number}'); // a number that looks like a letter, such as a Roman numeral.
  const otherNumber = X('\\p{Other_Number}'); // a superscript or subscript digit, or a number that is not a digit 0–9 (excluding numbers from ideographic scripts).
  const punctuation = X('\\p{Punctuation}'); // any kind of punctuation character.
  const dashPunctuation = X('\\p{Dash_Punctuation}'); // any kind of hyphen or dash.
  const openPunctuation = X('\\p{Open_Punctuation}'); // any kind of opening bracket.
  const closePunctuation = X('\\p{Close_Punctuation}'); // any kind of closing bracket.
  const initialPunctuation = X('\\p{Initial_Punctuation}'); // any kind of opening quote.
  const finalPunctuation = X('\\p{Final_Punctuation}'); // any kind of closing quote.
  const connectorPunctuation = X('\\p{Connector_Punctuation}'); // a punctuation character such as an underscore that connects words.
  const otherPunctuation = X('\\p{Other_Punctuation}'); // any kind of punctuation character that is not a dash, bracket, quote or connector.
  const other = X('\\p{Other}'); // invisible control characters and unused code points.
  const control = X('\\p{Control}'); // an ASCII or Latin-1 control character: 0x00–0x1F and 0x7F–0x9F.
  const format = X('\\p{Format}'); // invisible formatting indicator.
  const privateUse = X('\\p{Private_Use}'); // any code point reserved for private use.
  const surrogate = X('\\p{Surrogate}'); // one half of a surrogate pair in UTF-16 encoding.
  const unassigned = X('\\p{Unassigned}'); // any code point to which no character has been assigned.
  const emoji = X(emojiRegex());

  const common = X('\\p{Common}');
  const arabic = X('\\p{Arabic}');
  const armenian = X('\\p{Armenian}');
  const bengali = X('\\p{Bengali}');
  const bopomofo = X('\\p{Bopomofo}');
  const braille = X('\\p{Braille}');
  const buhid = X('\\p{Buhid}');
  const canadianAboriginal = X('\\p{Canadian_Aboriginal}');
  const cherokee = X('\\p{Cherokee}');
  const cyrillic = X('\\p{Cyrillic}');
  const devanagari = X('\\p{Devanagari}');
  const ethiopic = X('\\p{Ethiopic}');
  const georgian = X('\\p{Georgian}');
  const greek = X('\\p{Greek}');
  const gujarati = X('\\p{Gujarati}');
  const gurmukhi = X('\\p{Gurmukhi}');
  const han = X('\\p{Han}');
  const hangul = X('\\p{Hangul}');
  const hanunoo = X('\\p{Hanunoo}');
  const hebrew = X('\\p{Hebrew}');
  const hiragana = X('\\p{Hiragana}');
  const inherited = X('\\p{Inherited}');
  const kannada = X('\\p{Kannada}');
  const katakana = X('\\p{Katakana}');
  const khmer = X('\\p{Khmer}');
  const lao = X('\\p{Lao}');
  const latin = X('\\p{Latin}');
  const limbu = X('\\p{Limbu}');
  const malayalam = X('\\p{Malayalam}');
  const mongolian = X('\\p{Mongolian}');
  const myanmar = X('\\p{Myanmar}');
  const ogham = X('\\p{Ogham}');
  const oriya = X('\\p{Oriya}');
  const runic = X('\\p{Runic}');
  const sinhala = X('\\p{Sinhala}');
  const syriac = X('\\p{Syriac}');
  const tagalog = X('\\p{Tagalog}');
  const tagbanwa = X('\\p{Tagbanwa}');
  const taile = X('\\p{TaiLe}');
  const tamil = X('\\p{Tamil}');
  const telugu = X('\\p{Telugu}');
  const thaana = X('\\p{Thaana}');
  const thai = X('\\p{Thai}');
  const tibetan = X('\\p{Tibetan}');
  const yi = X('\\p{Yi}');
  ...
  class ScriptResolver {
      constructor(url) {
          this.resolvedScripts = Object.entries(scripts$1)
              .filter(script => {
  ...
  class SpoofChecker {
      ...
      safeToDisplayAsUnicode(label, isTldAscii) {
          ...
          // single script
          if (result === RestrictionLevel.SINGLE_SCRIPT_RESTRICTIVE &&
...
//# sourceMappingURL=ens-validation.umd.js.map
