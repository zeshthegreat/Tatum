...
exports.protoTypeToAminoType = exports.getObjectEip712PropertyType = exports.mapValuesToProperValueType = exports.stringTypeToReflectionStringType = exports.numberTypeToReflectionNumberType = exports.objectKeysToEip712Types = void 0;
...
const objectKeysToEip712Types = ({ object, messageType, primaryType = 'MsgValue', }) => {
    ...
    for (const property in (0, snakecase_keys_1.default)(object)) {
        const propertyValue = (0, snakecase_keys_1.default)(object)[property];
        if (property === '@type') {
            continue;
        }
        ...
        if (type === 'boolean') {
            ...
            types.push({
                name: property,
                type: (0, exports.numberTypeToReflectionNumberType)(property),
            });
        }
        else if (type === 'string') {
 ...
 */
const numberTypeToReflectionNumberType = (property) => {
    switch (property) {
        case 'order_mask':
            return 'int32';
 ...
 */
const mapValuesToProperValueType = (object, messageTypeUrl) => {
    ...
    return Object.keys(object).reduce((result, key) => {
        ...
        if (!value) {
            ...
            if (messageTypeUrl) {
                const typeInMap = Object.keys(nullableStringsTypeMaps).find((key) => key === messageTypeUrl);
                if (typeInMap) {
                    const nullableStringKeys = nullableStringsTypeMaps[typeInMap];
        ...
        if (typeof value === 'object') {
            if (value instanceof Date) {
                return Object.assign(Object.assign({}, result), { [key]: value.toJSON().split('.')[0] + 'Z' });
            }
            if (Array.isArray(value)) {
                return Object.assign(Object.assign({}, result), { [key]: value.every((i) => typeof i === 'string')
                        ? value
                        : value.map((item) => (0, exports.mapValuesToProperValueType)(item)) });
            }
            return Object.assign(Object.assign({}, result), { [key]: (0, exports.mapValuesToProperValueType)(value) });
        }
        if ((0, numbers_1.isNumber)(value)) {
        ...
        if (typeof value === 'string') {
            if (sdkDecKeys.includes(key)) {
                return Object.assign(Object.assign({}, result), { [key]: (0, numbers_1.numberToCosmosSdkDecString)(value) });
            }
            // Message Type Specific checks
            if (messageTypeUrl) {
                const typeInMap = Object.keys(sdkDecKeyWithTypeMaps).find((key) => key === messageTypeUrl);
                if (typeInMap) {
                    const sdkDecKeys = sdkDecKeyWithTypeMaps[typeInMap];
                    if (sdkDecKeys.includes(key)) {
                        return Object.assign(Object.assign({}, result), { [key]: (0, numbers_1.numberToCosmosSdkDecString)(value) });
                    }
                }
...
exports.mapValuesToProperValueType = mapValuesToProperValueType;
