...
function isEthChain(chainId) {
  const ethChains = /* @__PURE__ */ new Set([import_chains.mainnet.id, import_chains.goerli.id, import_chains.sepolia.id]);
...
function getChainFromId(chainId) {
  switch (chainId) {
    ...
    case 5:
      return import_chains.goerli;
    case 11155111:
      return import_chains.sepolia;
    default:
      return import_chains.mainnet;
  }
}
...
var ContractReader = class {
  ...
  getVerifiedTldHubContract() {
    const ethClient = (0, import_viem2.createPublicClient)({
      chain: this.isDev ? import_chains2.bscTestnet : import_chains2.mainnet,
      transport: (0, import_viem2.http)(this.isDev ? void 0 : this.rpcUrl)
    });
    const hubContract = (0, import_viem2.getContract)({
   ...
   * @return {*}
   */
  async getResolverContractByTld(namehash4, tldInfo, rpcUrl) {
    const client = createCustomClient(tldInfo, rpcUrl);
    const registryContract = (0, import_viem2.getContract)({
      address: tldInfo.registry,
      abi: SIDRegistryAbi,
      publicClient: client
    });
    const resolverAddr = await registryContract.read.resolver([namehash4]);
    if (!(0, import_viem2.hexToNumber)(resolverAddr)) {
      throw "resolver address is null";
    }
    const resolverContract = (0, import_viem2.getContract)({
  ...
  async getTldMetadata(domain, tldInfo, rpcUrl) {
    const tokenId = (0, import_viem2.hexToBigInt)((0, import_viem2.keccak256)(Buffer.from(domain.split(".")[0])));
    const client = createCustomClient(tldInfo, rpcUrl);
    const sannContract = (0, import_viem2.getContract)({
...
function tldNamehash(inputName, identifier) {
  if (!identifier)
    return (0, import_viem3.namehash)(inputName);
  const fullNameNode = `${inputName}.[${(0, import_viem3.toHex)(identifier, { size: 32 }).slice(2)}]`;
  return (0, import_viem3.namehash)(fullNameNode);
}
...
var UDResolver = class {
  ...
  async getAddress(domain) {
    const client = (0, import_viem5.createPublicClient)({
      chain: import_chains3.polygon,
      transport: (0, import_viem5.http)()
    });
    const proxyReaderContract = (0, import_viem5.getContract)({
      address: this.proxyReaderAddress,
      abi: proxyReaderAbi,
      publicClient: client
    });
    const keys = ["crypto.ETH.address"];
    const res = await proxyReaderContract.read.getMany([keys, BigInt((0, import_viem5.namehash)(domain))]);
  ...
  async getName(address) {
    ...
    const registryContract = (0, import_viem5.getContract)({
...
var Web3Name = class {
  ...
  async getDomainName({ address, queryChainIdList, queryTldList, rpcUrl }) {
    ...
    try {
      const reverseNode = `${(0, import_ens.normalize)(address).slice(2)}.addr.reverse`;
      const reverseNamehash = (0, import_viem6.namehash)(reverseNode);
  ...
  async getAddress(name, { coinType, rpcUrl } = {}) {
    ...
    const normalizedDomain = "lens" /* LENS */ === tld ? name : (0, import_ens.normalize)(name);
    ...
    try {
      ...
      const namehash4 = tldNamehash(normalizedDomain, isV2Tld(tld) ? void 0 : tldInfo.identifier);
      const resolverContract = await this.contractReader.getResolverContractByTld(namehash4, tldInfo, rpcUrl);
      const res = coinType !== void 0 ? await resolverContract.read.addr([namehash4, BigInt(coinType)]) : await resolverContract.read.addr([namehash4]);
      return res;
    } catch (error) {
  ...
  async getDomainRecord({ name, key, rpcUrl }) {
    ...
    try {
      ...
      const namehash4 = tldNamehash(normalizedDomain, isV2Tld(tld) ? void 0 : tldInfo.identifier);
      const resolverContract = await this.contractReader.getResolverContractByTld(namehash4, tldInfo, rpcUrl);
      const record = await resolverContract.read.text([namehash4, key]);
      return record;
    } catch (error) {
      console.error(`Error getting address for ${name}`, error);
      return null;
  ...
  async getContentHash({ name, rpcUrl }) {
    ...
    try {
      ...
      if (!tldInfo)
        throw "TLD not found";
      const namehash4 = tldNamehash((0, import_ens.normalize)(name), isV2Tld(tld) ? void 0 : tldInfo.identifier);
      const contenthash = await this.contractReader.getContenthash(namehash4, tldInfo, rpcUrl);
