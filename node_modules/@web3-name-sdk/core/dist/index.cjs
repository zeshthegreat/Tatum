...
var ContractReader = class {
  ...
  async getTldMetadata(domain, tldInfo, rpcUrl) {
    ...
    const tldBaseContract = (0, import_viem2.getContract)({
  ...
  async containsTldNameFunction(resolverAddr, tldInfo, rpcUrl) {
    ...
    const selector = (0, import_viem2.toFunctionSelector)("tldName(bytes32, uint256)");
    return bytecode?.includes(selector.slice(2)) ?? false;
  }
  async resolverFunctionExists(resolverAddr, functionName, tldInfo, rpcUrl) {
    const client = createCustomClient(tldInfo, rpcUrl);
    const bytecode = await client.getBytecode({ address: resolverAddr });
    const selector = (0, import_viem2.toFunctionSelector)(functionName);
    return bytecode?.includes(selector.slice(2)) ?? false;
  }
};
...
function tldNamehash(inputName, identifier) {
  if (!identifier)
    return (0, import_viem4.namehash)(inputName);
  const fullNameNode = `${inputName}.[${(0, import_viem4.toHex)(identifier, { size: 32 }).slice(2)}]`;
  return (0, import_viem4.namehash)(fullNameNode);
}
...
var UDResolver = class {
  ...
  async getAddress(domain) {
    ...
    const proxyReaderContract = (0, import_viem6.getContract)({
      address: this.proxyReaderAddress,
      abi: proxyReaderAbi,
      publicClient: client
    });
    const keys = ["crypto.ETH.address"];
    const res = await proxyReaderContract.read.getMany([keys, BigInt((0, import_viem6.namehash)(domain))]);
  ...
  async getName(address) {
    ...
    const registryContract = (0, import_viem6.getContract)({
...
var Web3Name = class {
  ...
  async getDomainName({ address, queryChainIdList, queryTldList, rpcUrl }) {
    ...
    try {
      ...
      const reverseNamehash = (0, import_viem7.namehash)(reverseNode);
  ...
  async getContentHash({ name, rpcUrl }) {
    ...
    try {
      ...
      if (!tldInfo)
        throw "TLD not found";
      const namehash5 = tldNamehash((0, import_ens.normalize)(name), isV2Tld(tld) ? void 0 : tldInfo.identifier);
